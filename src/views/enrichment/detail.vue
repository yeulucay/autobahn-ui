    ...
    <el-row>
      <el-col :span="6">
        <el-row class="row-margin">
          ...
          <br />
          <el-input v-model="enrichment.name" />
        </el-row>
        <el-row class="row-margin">
          <span class="label">Endpoint:</span>
          <br />
          <el-select class="select-style" v-model="enrichment.endPointId" placeholder="Select">
        ...
        <el-row class="row-margin">
          ...
          <br />
          <el-select class="select-style" v-model="enrichment.dataTypeId" placeholder="Select">
        ...
        <el-row class="row-margin">
          ...
          <br />
          <el-select class="select-style" v-model="enrichment.finalStorageId" placeholder="Select">
...
export default {
  ...
  methods: {
    saveEnrichment() {
      ...
      this.enrichmentModel.forEach(enrModel => {
        delete enrModel.columnNames; // delete columnNames from attrs.
        delete enrModel.selectedAttr;
        if (enrModel.joinModel) {
          enrModel.joinModel.forEach(jModel => {
            delete jModel.joinDestColumns;
            delete jModel.joinSourceColumns;
          });
        }
      });
      data["matchModel"] = this.enrichmentModel;
      data["selectedColumnList"] = this.totalList;

      this.enrichment.enrichmentRules = JSON.stringify(data);
      // console.log(this.enrichment);
      postEnrichment(this.enrichment);
    },
    organizeRules() {
      var detail = JSON.parse(this.enrichment.enrichmentRules);
      if (detail) {
        this.enrichmentModel = detail.matchModel;
        this.totalList = detail.selectedColumnList;
      }
    },
    clearRules() {
      this.enrichmentModel = [];
      this.totalList = [];
    },
    cancelEnrichment() {},
    addJoinModel(i) {
