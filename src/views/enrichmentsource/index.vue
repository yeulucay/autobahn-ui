  ...
  <div class="app-container">
    ...
    <el-row class="row-margin">
      <el-table border fit style="width: 100%"
        :data="enrichmentSourceList">
        <el-table-column prop="name" label="Name"/>
        <el-table-column prop="address" label="Address"/>
        <el-table-column prop="port" label="Port"/>
        <el-table-column prop="enrichmentSourceTypeCode" label="Type"/>
...
<script>
import {getEnrichmentSourceList} from '@/api/enrichment'
  ...
  methods: {
      ...
      getProjectList().then(response=>{
        if(response.data){
          ...
          if(this.projects.length > 0){
            ...
            this.getEnrichmentSources(this.selectedProject)
          }
        }
      })
    },
    getEnrichmentSources(projectId){
      getEnrichmentSourceList(projectId).then(response=>{
        if(response.data){
          this.enrichmentSourceList = response.data
        }
      })
    }
  }
